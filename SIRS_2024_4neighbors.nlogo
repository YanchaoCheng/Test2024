turtles-own [
  immunity-period ; the time the immunity lasts
  infectious-period; how long the infected-infectious status last
] ;
...
to infected-infectious
   ...
   [ if random 100 <= Beta ; infection probablity from one contact
    [
        ...
        ;ask one-of (turtles-on neighbors) with [color = green]
...
to susceptible
  ...
  [
    ifelse immunity-period <= 0
    [set color green]
  [set immunity-period immunity-period - 1]
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"" ""
PENS
"Susceptible" 1.0 0 -12087248 true "" "plot count turtles with [color = green]"
"Infected-infectious" 1.0 0 -5298144 true "" "plot count turtles with [color = red]"
"Recovered" 1.0 0 -14070903 true "" "plot count turtles with [color = blue]"
